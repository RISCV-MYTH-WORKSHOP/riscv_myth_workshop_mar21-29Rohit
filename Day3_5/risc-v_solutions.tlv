...
\TLV
   ...
   |cpu
      @0
         ...
         $pc[31:0] = >>1$reset ? 32'd0 : // NEXT PC
                     >>3$valid_taken_br ? >>3$br_tgt_pc :
                     >>3$valid_load ? >>3$inc_pc : (>>3$valid_jump && >>3$is_jalr) ? >>3$jalr_tgt_pc[31:0] : (>>3$valid_jump && >>3$is_jal) ? >>3$br_tgt_pc : >>1$inc_pc;
         
         //$start = (>>1$reset && $reset == 0) ? 1'b1 : 1'b0;
      
      @1
         $inc_pc[31:0] = $pc + 32'd4; // INCREMENT PC
         
         $imem_rd_en = ! $reset; // FETCH
         $imem_rd_addr[31:0] = $pc[M4_IMEM_INDEX_CNT+1:2];
         $instr[31:0] = $imem_rd_data;
         
         $is_u_instr = $instr[6:2] ==? 5'b0x101; // INSTRUCTION TYPE DECODE
         ...
         $imm[31:0] = $is_i_instr ? { {21{$instr[31]}}, $instr[30:20] } : // Instruction Immediate Decode
      ...
      @2
         $rf_rd_en1 = $rs1_valid;// REGISTER READ FILE
         $rf_rd_en2 = $rs2_valid;
         ...
         $br_tgt_pc[31:0] = $pc + $imm; // BRANCH AND JUMP
         $jalr_tgt_pc[31:0] = $src1_value[31:0] + $imm;
         $is_jump = $is_jal || $is_jalr;
         
         
      @3
         $sltu_rslt[31:0] = $src1_value < $src2_value; // COMPLETE ALU
         ...
         $taken_br = $is_beq ? ($src1_value == $src2_value) : // BRANCH CONTAINING OPERATION WITH ALU OUTPUTS 
         ...
         $valid = !(>>1$valid_taken_br || >>2$valid_taken_br || // VALID STATEMENT FOR BRANCH AND LOAD AND JUMP
                    >>1$valid_load || >>2$valid_load || >>1$valid_jump || >>2$valid_jump);           

         
         $rf_wr_en =  $rd_valid && ($rd != 5'b0) && $valid; // REGISTER WRITE
         $rf_wr_index[4:0] = >>2$valid_load ? >>2$rd : $rd;
         $rf_wr_data[31:0] = >>2$valid_load ? >>2$ld_data : $result;
         

         $valid_taken_br = $valid && $taken_br; // VALID BRANCH AND LOAD And JUMP
         $valid_load = $valid && $is_load;
         $valid_jump = $valid && $is_jump;
      
      @4
         $dmem_wr_en = $is_s_instr && $valid; // DMEM MEMORY
         $dmem_rd_en = $is_load;
         $dmem_addr[3:0] = $result[5:2];
         $dmem_wr_data[31:0] = $src2_value;
         
      @5
         $ld_data[31:0] = $dmem_rd_data; // OUTPUT OF DMEM MEMORY
